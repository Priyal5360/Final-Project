...
class WooCommerceCheckOutForm extends FormInterface
{
	...
	jQuery("input[name=payment_method]").click(function() {   onChangePayment(jQuery("input[name=payment_method]:checked").val()); 
	});   
	...
	function onChangePayment(payment)
	{
		if (!post_verify)
		{
		if ((ask_otp && ((jQuery.inArray(payment, paymentMethods) > -1) || !selected_payment)) || ((1 == jQuery(".woocommerce #createaccount").prop("checked") || 0 == jQuery(".woocommerce #createaccount").length) && register_otp && !user_logged_in))
		{
			addShortcode();
		} else {
			removeShortcode();
